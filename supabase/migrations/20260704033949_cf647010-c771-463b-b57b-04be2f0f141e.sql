...
BEGIN
  ...
  -- Showings (no detail route yet -> link to list)
  RETURN QUERY
  SELECT
    ('showing:' || s.id::text),
    'pm_showings',
  ...
  -- Inspections (detail route exists)
  RETURN QUERY
  SELECT
  ...
  SELECT
    ...
    m.assigned_to,
    m.id,
    '/property-management/move-ins'::text
  ...
  -- Move-Outs (no detail route -> list)
  RETURN QUERY
  SELECT
    ('move_out:' || mo.id::text),
    ...
    mo.assigned_to,
    mo.id,
    '/property-management/move-outs'::text
  ...
  -- Lease Renewals (no detail route -> list)
  RETURN QUERY
  SELECT
    ...
    r.assigned_to,
    r.id,
    '/property-management/lease-renewals'::text
  ...
  -- Staff Tasks (no detail route -> list)
  RETURN QUERY
  SELECT
  ...
  -- Owner Approvals (no detail route -> list; full staff only)
  IF v_full THEN
    RETURN QUERY
    SELECT
      ...
      NULL::uuid,
      a.id,
      '/property-management/owner-approvals'::text
  ...
  -- Work Orders (detail route exists)
  RETURN QUERY
  SELECT
