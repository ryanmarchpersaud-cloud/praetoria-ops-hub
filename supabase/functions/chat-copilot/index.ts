...
// and NO AI-gateway work unless LEGACY_CHAT_COPILOT_ENABLED is explicitly "true".
import { serve } from "https://deno.land/std@0.168.0/http/server.ts";
import { createClient } from "https://esm.sh/@supabase/supabase-js@2.49.4";
import { requireAuth, requireRole, corsHeaders } from "../_shared/auth.ts";
import { legacyChatCopilotDisabledResponse } from "./legacyGate.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    const disabledResponse = legacyChatCopilotDisabledResponse(
      Deno.env.get("LEGACY_CHAT_COPILOT_ENABLED"),
      corsHeaders,
    );
