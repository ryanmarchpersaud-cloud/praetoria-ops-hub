...
export function useDashboardQuotes() {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('quotes')
        .select('id, approval_status, total, created_at, quote_number, follow_up_due_at, sent_status, sent_at, leads(first_name, last_name, company_name)')
        .order('created_at', { ascending: false });
      if (error) throw error;
      return data ?? [];
    },
...
export function useDashboardJobs() {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('jobs')
        .select('id, status, job_title, job_number, scheduled_date, customer_id, assigned_to, created_at, service_category, billing_status')
        .order('created_at', { ascending: false });
      if (error) throw error;
      return data ?? [];
    },
