...
import { FileText, Image as ImageIcon, Download, Video, PhoneOff, Phone } from 'lucide-react';
...
export const MessageBubble = React.forwardRef<HTMLDivElement, Props>(
  function MessageBubble({ message, showSender = true }, ref) {
    const { user } = useAuth();
    ...
    if (isSystem) {
      return (
        <div ref={ref} className="flex items-center justify-center my-2">
          <span className="text-[11px] text-muted-foreground bg-muted px-3 py-1 rounded-full flex items-center gap-1.5">
            {message.body?.includes('started') && <Video className="h-3 w-3" />}
            {message.body?.includes('ended') && <PhoneOff className="h-3 w-3" />}
            {message.body?.includes('Joined') && <Phone className="h-3 w-3" />}
            {message.body}
          </span>
        </div>
      );
    ...
    return (
      <div ref={ref} className={cn('flex gap-2 max-w-[85%]', isOwn ? 'ml-auto flex-row-reverse' : '')}>
        {showSender && !isOwn && (
