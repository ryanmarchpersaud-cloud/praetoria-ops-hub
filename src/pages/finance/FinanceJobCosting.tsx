...
export default function FinanceJobCosting() {
  ...
  const { data: jobs, isLoading } = useQuery({
    ...
    queryFn: async () => {
      // Fetch jobs with their invoices and expenses
      const { data: jobsData, error } = await supabase
        .from('jobs')
        .select('id, job_number, job_title, service_category, customer_id, property_id, customers(first_name, last_name, company_name), properties(address_line_1)')
        .order('created_at', { ascending: false });
      if (error) throw error;

      // Get revenue from invoices and costs from expenses for each job
      ...
      return (jobsData || []).map((j: any) => {
        ...
        return { ...j, revenue, costs, margin, marginPct };
      });
    },
  });
  ...
  const filtered = (jobs ?? []).filter((j: any) => {
    ...
    return j.job_number?.toLowerCase().includes(s) || j.job_title?.toLowerCase().includes(s) || j.customers?.first_name?.toLowerCase().includes(s) || j.customers?.last_name?.toLowerCase().includes(s);
  });
  ...
  const exportCSV = () => {
    const rows = filtered.map((j: any) => [j.job_number, j.job_title, j.service_category, j.revenue, j.costs.labor, j.costs.materials, j.costs.sub, j.costs.fuel, j.costs.other, j.costs.total, j.margin, j.marginPct.toFixed(1)].join(','));
    const csv = ['Job #,Title,Category,Revenue,Labor,Materials,Subcontractor,Fuel,Other,Total Cost,Margin,$Margin %', ...rows].join('\n');
                ...
                <TableBody>
                  {filtered.map((j: any) => (
                    <TableRow key={j.id}>
                      ...
                      <TableCell className="max-w-[200px] truncate">{j.job_title}</TableCell>
