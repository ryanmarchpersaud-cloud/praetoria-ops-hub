...
import { ArrowLeft, MapPin, Clock, Users, Eye, Stethoscope, User, Image as ImageIcon } from 'lucide-react';
...
export default function WorkerIncidentDetailPage() {
  ...
  if (!report) {
    return (
      <div className="px-4 pt-6 pb-4 text-center">
        ...
        <Link to="/worker/incidents"><Button variant="link">Back to reports</Button></Link>
      </div>
    );
  }
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-sm font-medium">
                {format(new Date(report.date_time), 'EEEE, MMMM d, yyyy')}
              </p>
              <p className="text-xs text-muted-foreground">
                {format(new Date(report.date_time), 'h:mm a')}
              </p>
            </div>
          </div>
          {report.location && (
      ...
      <Card>
        <CardContent className="p-4 space-y-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm">
              Medical Attention: <strong>{report.medical_attention ? 'Yes' : 'No'}</strong>
            </p>
          </div>
          {report.reported_to && (
            <div className="flex items-center gap-2">
