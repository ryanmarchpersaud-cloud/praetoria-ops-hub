...
export default function OwnerMessages() {
  const { data: threads = [], isLoading } = useOwnerOwnThreads();
  ...
  return (
    <OwnerLayout>
      <div className="p-4 space-y-3">
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[11px] text-muted-foreground mt-0.5">
                      ...
                      {t.last_owner_visible_message_at ? ` · ${formatDistanceToNow(new Date(t.last_owner_visible_message_at), { addSuffix: true })}` : ''}
                    </p>
                  </div>
        ...
        <OwnerThreadDialog thread={openThread} onClose={() => setOpenThread(null)} />
        <NewOwnerThreadDialog open={openNew} onOpenChange={setOpenNew} />
      </div>
    </OwnerLayout>
  );
}

function OwnerThreadDialog({ thread, onClose }: { thread: OwnerThread | null; onClose: () => void }) {
  const { data: messages = [], isLoading } = useThreadMessages(thread?.id, { ownerVisibleOnly: true });
  const reply = useOwnerReply();
  const markRead = useOwnerMarkThreadRead();
  const [body, setBody] = useState('');

  // Mark read when opened
  useMemo(() => { if (thread?.id) markRead.mutate(thread.id); /* eslint-disable-next-line */ }, [thread?.id]);
...
function NewOwnerThreadDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  const { data: properties = [] } = useOwnerProperties();
  const [propertyId, setPropertyId] = useState('');
  const [subject, setSubject] = useState('');
  const [category, setCategory] = useState<OwnerThreadCategory>('general');
  const [body, setBody] = useState('');
  const mut = useOwnerOpenThread();

  const submit = async () => {
    if (!propertyId) return toast.error('Select property');
    if (!subject.trim()) return toast.error('Subject required');
    if (!body.trim()) return toast.error('Message required');
    try {
      await mut.mutateAsync({ property_id: propertyId, subject, body, category });
      toast.success('Message sent');
      onOpenChange(false);
      setPropertyId(''); setSubject(''); setBody(''); setCategory('general');
    } catch (e: any) { toast.error(e.message); }
  };
        ...
        <div className="space-y-3">
          <div>
            ...
            <Select value={propertyId} onValueChange={setPropertyId}>
              ...
              <SelectContent>{(properties as any[]).map(p => (
                <SelectItem key={p.id} value={p.id}>{p.property_name || p.name || 'Property'}</SelectItem>
              ))}</SelectContent>
            </Select>
          </div>
          <div>
